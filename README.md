...
- **🧷 剧本连续性**：记录核心设定、分幕结构、场景节点、线索网络、NPC 剧本功能、威胁时钟和结局条件。
- **💾 可迁移存档**：玩家可随时导出 `.cocsave.json` 单文件存档；存档会内置续跑摘要，复制到另一台电脑后可导入继续游玩。
...
# 追加剧本连续性信息
python scripts/memory.py add --name "榕渊" --section outline --text "核心设定：榕树根系连接旧祭祀入口，潮湿手印是被吞没者的回声。"
python scripts/memory.py add --name "榕渊" --section scene-node --text "旧仓山洋楼二层：入口是楼梯，出口指向地下室；关键线索是墙纸下的手印。"
python scripts/memory.py add --name "榕渊" --section clue-web --text "潮湿手印 -> 地下室 -> 1998 年火灾记录 -> 陈姨隐瞒祭祀入口。"
python scripts/memory.py add --name "榕渊" --section threat-clock --text "每经过 2 个场景，根系污染扩散一层；第 4 阶段触发夜间集体梦境。"
python scripts/memory.py add --name "榕渊" --section ending --text "结局条件：烧毁根系可暂时封门；交出祭品会换取失踪者线索但扩大污染。"
...
python scripts/memory.py save --name "榕渊" --output "saves/rongyuan-before-basement.cocsave.json"

# 在另一台电脑导入存档继续
python scripts/memory.py load --file "saves/rongyuan-before-basement.cocsave.json"
python scripts/memory.py recall --name "榕渊" --keeper

# 导入为分支路线
python scripts/memory.py load --file "saves/rongyuan-before-basement.cocsave.json" --name "榕渊-地下室分支"
```
...
- `.cocsave.json` 是可复制到其他电脑的完整存档包，内含玩家可知续跑摘要、Keeper 续跑摘要和剧本连续性记忆。
...
- 玩家说“存档”“保存进度”“读档”“导入存档”时，先处理存档/读档，不继续推进剧情。
...
- **剧本连续性记忆**：核心设定、分幕结构、场景节点、线索网络、NPC 剧本功能、威胁时钟、结局条件和不能冲突的事实。
...
- 剧本节点、线索网络、NPC 剧本功能、威胁时钟或结局条件发生变化时立即更新。
- 用户说“继续”或上下文变长时，先回忆记忆文件，再继续叙事。

### 剧本连续性记忆

剧本相关记忆默认是 Keeper Only，用于确保长团不会只记得“玩家到了哪里”，却忘记“剧本为什么成立”。

推荐记录：

- **核心设定 / 剧本前提**：怪异事件真实原因、幕后规则、神话或非神话解释。
- **分幕结构**：开端、调查中段、转折、高潮、结局阶段，以及当前位于哪一幕。
- **场景节点**：重要地点的入口、出口、可发现线索、风险、已触发变化。
- **线索网络**：关键推论的多条线索路径，哪些已公开、哪些仍隐藏。
- **NPC 剧本功能**：盟友、阻碍、红鲱鱼、信息持有者、受害者、幕后相关者。
- **威胁时钟**：玩家不行动时，仪式、追踪者、敌方行动、污染或倒计时如何推进。
- **结局条件**：不同玩家选择可能导向的结局、失败条件和开放式收束方式。

常用命令：

```bash
python scripts/memory.py add --name "榕渊" --section outline --text "核心设定：..."
python scripts/memory.py add --name "榕渊" --section act --text "当前处于第二幕：调查转向地下空间。"
python scripts/memory.py add --name "榕渊" --section scene-node --text "洋楼二层 -> 地下室入口。"
python scripts/memory.py add --name "榕渊" --section clue-web --text "墙纸手印 -> 火灾记录 -> 祭祀入口。"
python scripts/memory.py add --name "榕渊" --section npc-function --text "陈姨：信息持有者 + 阻碍者。"
python scripts/memory.py add --name "榕渊" --section threat-clock --text "污染每 2 个场景推进 1 阶段。"
python scripts/memory.py add --name "榕渊" --section ending --text "封门、交易、失败扩散三类结局。"
python scripts/memory.py add --name "榕渊" --section continuity --text "手印只能从潮湿墙面出现，不能突然变成血字。"
```
...
# 导出到指定文件名
python scripts/memory.py save --name "榕渊" --output "saves/rongyuan-session-01.cocsave.json"

# 在另一台电脑继续：把 .cocsave.json 复制到 saves/ 后导入
python scripts/memory.py load --file "saves/rongyuan-session-01.cocsave.json"

# 想走分支路线时，导入为新团名
python scripts/memory.py load --file "saves/rongyuan-session-01.cocsave.json" --name "榕渊-分支A"
```

`.cocsave.json` 是完整存档，包含玩家可知信息、Keeper Only 真相、暗骰、隐藏 SAN、倒计时和伏笔。导出时还会生成两份续跑摘要：

- **玩家可知续跑摘要**：已进行流程、当前落点、主角状态、已知线索、NPC 公开状态、未解决问题。
- **Keeper 续跑摘要**：幕后真相、剧本当前节点、线索网络、NPC 隐藏动机、未公开线索、暗骰、隐藏 SAN、倒计时、伏笔和结局条件。
...
不建议玩家直接打开 `.cocsave.json` 阅读，避免被幕后信息剧透。
